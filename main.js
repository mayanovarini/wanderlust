// Foursquare API Info
const clientId = "1QSCS0UKRKD1NT0BLQG5YJSEFCPF1WF13P130XWN43ZUABVT";
const clientSecret = "55DRHUVOP5DHOSM4NCKACZTXJ0SKWVYGXAPFS3T33SK11BUR";
...
const $venueDivs = [$("#venue6"), $("#venue1"), $("#venue4"), $("#venue2"), $("#venue5"), $("#venue3")];
const $weatherDivs = [
  $("#weather1"),
  $("#weather2"),
  $("#weather3"),
...
function renderVenues(venues) {
  $venueDivs.forEach(($venue, index) => {
    let venueContent = '<h2>' + venues[index].name + '</h2>' +
                       '<img class="venueimage" src="' + imgPrefix + venues[index].photos.groups[0].items[0].suffix + '"/>' +
                       '<h3>Address:</h3>' + '<p>' + venues[index].location.address + '</p>' +
...
// changes on APIXU data structure, removed hour property and just use available properties form the day object
function renderForecast(days) {
  console.log("days", days);

  $weatherDivs.forEach(($day, index) => {
    let weatherContent = '<h2> High: ' + days[index].day.maxtemp_c + '</h2>' +
                         '<h2> Low: ' + days[index].day.mintemp_c + '</h2>' +
                         '<h2> Humidity: ' + days[index].day.avghumidity + '</h2>' +
...
async function getVenues() {
  ...
  const urlToFetch = url + city + '&venuePhotos=1&limit=10&client_id=' + clientId + "&client_secret=" + clientSecret + "&v=20171017";
